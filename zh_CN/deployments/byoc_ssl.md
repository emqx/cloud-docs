...
EMQX Cloud BYOC 部署使用 TLS/SSL 加密协议，建立安全的加密通信通道，在数据传输过程中保护数据的机密性。本文档概述了如何配置和使用单向 TSL/SSL 协议认证，确保 MQTT 系统的安全通信。
...
要为您的 BYOC 部署配置 TLS/SSL，请确保拥有以下组件并合并到单个 PEM 文件中：

1. **服务器证书**：这是由受信任的证书颁发机构（CA）颁发的证书，使 EMQX MQTT 代理能够与客户端建立安全连接。

2. **证书链**：包含完整的证书链，直至受信任的根 CA 证书。这使客户端设备能够验证服务器证书的真实性。

3. **私钥**：与服务器证书关联的私钥是解密客户端与 EMQX MQTT 代理之间通信的关键。
...
在执行 `./byoc create` 命令部署您的 BYOC 环境时，参数中需提供包含服务器证书、证书链和私钥的 PEM 文件路径。这个 PEM 文件对于启用 MQTT 客户端与 EMQX MQTT 代理之间的安全单向认证至关重要。

在执行 `./byoc create` 命令之前，请确保已准备好 PEM 文件，并且可以轻松访问。