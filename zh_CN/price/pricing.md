...
**连接费：** 以连接分钟进行计算，1 个连接分钟为 1 个客户端连接到部署 1 分钟的计量单位，不足 1 分钟以 1 分钟计算。
...
**连接费**：
$$
连接费=客户端连接数×连接持续时间（分钟，向上取整）÷1,000,000×2
$$
**流量费**：
$$
流量费=进出流量（字节）÷(1024×1024×1024)×0.15
$$
**数据集成**：
$$
数据集成费用=规则动作执行次数÷1,000,000×0.25
$$
::: tip 连接费计费示例
在 24 小时的计费周期内，假设用户有以下客户端连接：
...
按小时计费时，费用按前一小时内专有版部署消费情况计算，每小时统计核算一次（小时账单），并从您的账户余额中扣费，然后累加到当月消费（月账单），您可以前往[账单概览](../billing/overview.md)查看详细扣费信息。
...
年度预付选项允许您选择通过信用卡或可用积分付款。选择年度预付选项时，您将享受总费用的 15% 折扣。在部署创建过程中，反映此折扣的预估总费用将在概要部分显示。
...
**基础费用**：基于您分配资源的年度订阅费用，确保全年提供专用容量。
...
**年度订阅**：旗舰版采用年度计费周期，基础费用涵盖您整年所需的专用资源分配。

**基于流量的收费**：月度使用费用根据您的入站和出站数据流量计算，提供清晰且可预测的费用结构。
