...
EMQX Platform 提供四种不同的产品版本，让您能够在全球热门的公共云上部署完全托管的 MQTT 服务。无论您是独立开发者还是全球行业领袖，您都能找到适合您需求的 EMQX Platform 产品。
...
### 高可用性与数据安全
EMQX Platform 专有版、旗舰版和 BYOC 均采用高冗余集群架构，以确保服务的高可用性；通过独有隔离环境保障您数据安全与业务稳定性，每个部署集群都有自己的公共 IP、专用 VPC 网络、独立 EMQX 服务器与数据库服务器，这更安全，更可靠。


### 灵活的数据集成方案
EMQX Platform 专有版、旗舰版和 BYOC 支持实时预处理设备事件与消息数据，并存储到云上 MySQL、Kafka、InfluxDB 等 40+ 的数据库与消息队列资源中。
