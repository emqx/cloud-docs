...
本文将以 MQTT.fx 作为 MQTT 客户端测试工具，接入 MQTT 云服务 - EMQ X Cloud 部署。
...
EMQ X Cloud 所有部署均开启了用户认证，因此使用 MQTT.fx 进行测试连接时，**需要填写用户名和密码字段**。
...
更多订阅、发布等功能请参考：[使用 MQTT.fx 接入 EMQ X Cloud](https://www.emqx.com/zh/blog/connecting-to-emqx-cloud-with-mqttfx)
