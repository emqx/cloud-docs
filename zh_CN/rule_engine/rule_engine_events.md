...
EMQ X Cloud Rule Engine (以下简称规则引擎) 提供了以下七个可用的新增事件订阅主题。
...
本文将从 设备上下线通知，设备消息通知和设备主题订阅通知三个方面分别介绍新增事件订阅主题使用和各个字段的含义。
...
设备上线的消息主题为 `$events/client_connected`，在规则面板中点击创建规则，然后输入如下规则匹配 SQL 语句便可完成设备上线规则的创建，当新设备上线并连接到EMQ X Cloud便会触发该规则。
   ```sql
    SELECT
        *
    FROM
...
使用SQL测试，查看触发该规则后返回字段，也可在 SQL 语句中使用 SELECT 和 WHERE 子句进行筛选与过滤。
