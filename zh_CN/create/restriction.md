...
| --------------------| ----------------------- | ------------------|----------------|
| 内部认证鉴权最大条目数     | 部署连接规格 * 2      | 可调整 [注1]                 |不需重启|
| 内部访问控制最大条目数     | 部署连接规格 * 2      | 可调整 [注2]                  |不需重启|
| 同时支持的外部扩展认证     | 2种      | 不可调整                   |-|
...
[注1] 最高可调整至部署连接数，超过 10 万条目 建议外部数据库认证。<br>
[注2] 最高可调整至部署连接数，超过 10 万条目 建议外部数据库认证。<br>
[注3] 最高可调整至部署连接数。<br>
[注4] 最大可调整至 256MB，需要注意修改该数值将会受到带宽限制导致 TPS 下降。<br>
