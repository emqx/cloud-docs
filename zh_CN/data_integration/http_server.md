...
5. 配置以下信息：
   ...
   - **URL 路径**：可以设置动作单独的请求路径，此路径将附加到连接器的 url 配置中，以形成完整的 URL 地址。此选项中可以使用带有变量的模板。您需要首先在规则SQL中定义相关变量，例如，`“select clientid as device_id”`。之后，您可以在HTTP动作的URL路径中使用变量`“${device_id}”`。
