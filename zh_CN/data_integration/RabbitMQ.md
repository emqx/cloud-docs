...
![EMQX Cloud-RabbitMQ 集成](./_assets/data_integraion_rabbitmq.jpg)
...
5. 完成消息从 EMQX Cloud 到发布到 RabbitMQ 的配置：
     ...
     - **有效载荷模版**： 默认值为空字符串，意味着将被转发至 RabbitMQ 服务器的消息载荷是未经任何修改的 JSON 格式文本。
       ...
       ```
        ...
        该模板将生成一个包含传入 MQTT 消息的载荷和时间戳的 JSON格式消息。其中，`${payload}` 和 `${timestamp}` 为占位符，当消息转发到 RabbitMQ 服务器时，它们将被实际的值替换。
