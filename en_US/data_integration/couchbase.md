# Writing MQTT Data to Couchbase
...
Couchbase data integration is an out-of-the-box feature in EMQX, designed to combine the real-time data capture and transmission capabilities of MQTT with the powerful data processing capabilities of Couchbase. Through the built-in [rules engine](https://docs.emqx.com/en/emqx/latest/data-integration/rules.html) component, this integration simplifies the process of importing data from EMQX into Couchbase for storage and analysis, without complex coding.

The diagram below illustrates a typical architecture for EMQX and Couchbase data integration.
...
- **Real-time Data Stream**: EMQX is designed for handling real-time data streams, ensuring efficient and reliable data transfer from source systems to Couchbase. This enables organizations to capture and analyze data in real time, making it ideal for use cases that require immediate insights and actions.
...
- **Flexibility in Data Transformation**: EMQX provides a powerful SQL-based rules engine that allows organizations to preprocess data before storing it in Couchbase. It supports various data transformation mechanisms such as filtering, routing, aggregation, and enrichment, helping organizations tailor data according to their needs.
- **Ease of Deployment and Management**: EMQX offers a user-friendly interface for configuring data sources, preprocessing data rules, and Couchbase storage settings, simplifying the setup and ongoing management of the data integration process.
...
1. Start the Couchbase server with the following command.
   The server needs to have these ports open: 8093 (for connecting and inserting data), 8091 (for accessing the Web UI).
...
2. Open the Couchbase Web console in your browser at http://x.x.x.x:8091.
    ...
    ① Click **Setup New Cluster** and name your cluster. For ease of getting started, set the full admin credentials to admin and password.

    Then accept the terms and conditions and click **Finish with Defaults** to complete the configuration using default values.

    ② After entering the configuration information, click the **Save & Finish** button at the bottom right. This will set up the server according to the configuration and open the Couchbase Web console dashboard.

    ③ In the left navigation panel, select **Buckets**, then click the **ADD BUCKET** button, enter a name for the bucket, such as `emqx`, and click **Create** to create the bucket.

    ④ Create a primary index for the default collection:
...
#### Creating a Couchbase Service with Couchbase Cloud
...
Now, you have successfully created a Couchbase Cloud instance and can proceed with the connection test.
...
2. In the SQL editor, enter the rule. The engine will be triggered when clients send temperature and humidity messages to the `temp_hum/emqx` topic. The SQL requires some adjustments:
